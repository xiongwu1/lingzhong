 ...
 (function($){
	...
	$.extend($.PaginationRenderers.defaultRenderer.prototype, {
		...
		getLinks:function(count, current_page, eventHandler) {
			...
			/*if(this.opts.show_goto_btn){
				fragment.prepend('<label><i>' + count + '</i>条</label>');
			}*/
			fragment.prepend('<label>共<i>' + count + '</i>条</label>');
			// 跳转
			if(this.opts.show_goto_btn){
				fragment.append('<label>跳转到<em><input type="text" name="page_num" value="' + 
					((count > 0) ? (current_page + 1) : '') + '" /></em>页</label><span class="goto">GO</span>');
			}
			$('a', fragment).click(eventHandler);
			$('.goto', fragment).click(eventHandler);
			return fragment;
		}
	});
	...
	$.fn.pagination = function(maxentries, opts){
		...
		function paginationClickHandler(evt){
			if(evt.target.tagName == "BUTTON" || $(evt.target).hasClass('goto')){
				...
				if(!isNaN(goto_current_page)){
					...
					if(goto_current_page > 0 && goto_current_page <= np) {
						...
						Util.Tip.warning("请输入正确的数字");
						$goto_page.val("");
						return;
					}
				}else{
					Util.Tip.warning("请输入数字");
					$goto_page.val("");
					return;
				}
			}else{
		...
		//输入限制
		containers.find('input').limited({type: 'int', min: 1, max: np});
